import MusicAndDanceDetail from "Components/DetailPages/MusicAndDanceDetail/MusicAndDanceDetail";
...
function Details({ stringedData, SimilarProd }) {
...
export const getServerSideProps = wrapper.getServerSideProps(
  (store) =>
    async ({ params, req }) => {
      ...
      let result_single = await VendorUserModels.findById(id);
      ...
      const condition_many = {
        ...
        subSubCategory: result_single?.subSubCategory,
      };
      const page = 0;
      const skip = 40;
      let result_many = await VendorUserModels.find(condition_many)
        .sort({ priority: 1 })
        .skip(page * skip)
        .limit(skip);
      const total = await VendorUserModels.countDocuments(condition_many);
      let res1;
