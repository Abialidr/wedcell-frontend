...
const SingleTodo = ({
  ...
  const s = globleuser?.data?.adminMessageId
    ...
    <>
      <div
        ...
        style={{
          ...
          gridGap: "0px",
          padding: "10px 5%",
          background: selected && selected._id == s ? "#ffb9ca29" : "",
        }}
      >
        <div
          onClick={() => {
            ...
            if (selected && selected._id == s) {
            } else {
              setALLmessages([]);
              setModifiedMsgs([]);
              setSelected({
                vendorName: "Wedfield Admin ",
                vendorContact: "919910990378",
                vendorImage: `/public/Layout/logo.svg`,
              ...
              >
                <article style={{ width: "100%", justifyContent: "start" }}>
                  <div className={styles.imgmessagediv}>
                    <img
                      className={styles.imgmessagediv}
                      src={`${S3PROXY}${values.vendorImage}`}
                      alt={values.vendorName.substring(0, 1)}
                    />
...
const Message = () => {
      ...
      messages.forEach((message) => {
        ...
        const name =
          message.senderId == globleuser?.data?._id
            ...
            : message.senderId == selected.vendorId
            ? selected.vendorName
            : "Admin Wedfield";
  ...
  const getMsg = async () => {
    ...
    if (result.data.newUser) {
      ...
      localStorage.setItem("wedfield", JSON.stringify(u));
      location.reload();
    }
    setPage((val) => ++val);
  ...
  const sendMessage = async (fileurl, msgtype) => {
    ...
    let socketBody = {
      ...
      messageBody: {
        ...
        receiverId: ["648d3c8efb95751e4d881bee", selected.vendorId],
        time: new Date(Date.now()).toLocaleTimeString([], {
          hour: "2-digit",
          minute: "2-digit",
        }),
  ...
  const initialSelect = () => {
    if (initialQuerySelect) {
      setSelected(uSERLIST.find((item) => item._id == router.query.id));
      setInitialQuerySelect(false);
    }
  };
  useEffect(() => {
    uSERLIST?.length && router?.query?.id && initialSelect();
  }, [uSERLIST]);
  useEffect(() => {
    latest &&
      selected &&
                      ...
                      {selected && (
                        <div className={styles.messagediv}>
                          ...
                          >
                            <div className={styles.messagedivheadleft}>
                              {selected && selected.vendorId ? (
                                <img
                                  ...
                                  src={`${S3PROXY}${selected.vendorImage}`}
                            ...
                            {/* <div
                              onClick={() => {
                                window.open(`tel://+${selected.vendorContact}`);
                              }}
                              className={styles.messagehead3dots}
                            >
                              <img
                                src={`${S3PROXY}/public/images/phone.png`}
                                alt=""
                              />
                              Call
                            </div> */}
